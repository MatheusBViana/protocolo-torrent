...
class Node:
    ...
    def upload(self, file_name: str):
        if not self.check_file_exist(file_name):
            print(f'The file you are trying to upload doesn\'t exist')
            return
        self.chunks[file_name] = self.get_chunks(file_name)
        data = {
            'type': 'add_peer',
            'peer_addr': self.addr,
            'file_name': file_name,
        ...
        self.send_receive_message_to_tracker(data, has_response=False)
    ...
    def search(self, file_name: str):
        ...
        response = self.send_receive_message_to_tracker(data, has_response=True)
        peers, chunk_count = self.parse_tracker_response(response)
        if peers:
            self.download(file_name, peers, chunk_count)
    ...
    @staticmethod
    def get_chunks(file_name) -> List[bytes]:
        file_path = UPLOAD_DIR + file_name
